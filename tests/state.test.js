...
import { loadState, saveItems, saveSettings, MAX_ITEMS, SUBCATEGORIES, ALL_TYPES, DEFAULT_SETTINGS } from '../script.js';
...
describe('Save/load item with contribution, withdrawal, loan, and retirement401k fields', () => {
  ...
  it('persists and restores retirement401k sub-object', () => {
    const item = {
      ...
      loan: null,
      retirement401k: {
        employeeContribution: 20000,
        employerMatchPct: 50,
        employerMatchCapPct: 6,
        annualSalary: 120000,
        vestingYears: 3,
        withdrawalStartYear: 2055
      }
    };
    saveItems([item]);
    const loaded = loadState();
...
describe('loadState() merges tax settings with defaults when fields are missing/corrupt', () => {
  it('fills in missing tax fields with defaults', () => {
    // Save settings with only partial tax object
    const partialSettings = {
      ...
      tax: {
        filingStatus: 'married_filing_jointly'
        // birthYear, annualSocialSecurityBenefit, socialSecurityStartYear, bracketInflationRate all missing
      }
    };
