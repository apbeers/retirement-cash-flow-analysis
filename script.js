...
// Section Add Button Logic
const addSectionBtn = document.getElementById('addSectionBtn');
const sectionHeader = document.getElementById('sectionHeader');

function updateSectionHeader() {
  // Set header and Add button based on current section
  let sectionTitle = 'Retirement Asset Projection';
  let addBtnLabel = '';
  addSectionBtn.classList.add('d-none');
  if (currentSection && currentSection !== 'dashboard') {
    sectionTitle = capitalize(currentSection);
    // Handle plural and singular for button label
    let singular = currentSection;
    if (currentSection === 'bank') singular = 'Bank Account';
    else if (currentSection === 'inflows') singular = 'Inflow';
    else if (currentSection === 'outflows') singular = 'Outflow';
    else if (currentSection === 'investments') singular = 'Investment';
    else if (currentSection === 'property') singular = 'Property';
    else if (currentSection === 'vehicles') singular = 'Vehicle';
    else if (currentSection === 'rentals') singular = 'Rental';
    addBtnLabel = `+ Add ${singular}`;
    addSectionBtn.textContent = addBtnLabel;
    addSectionBtn.classList.remove('d-none');
  }
  sectionHeader.querySelector('h1').textContent = sectionTitle;
}

addSectionBtn.addEventListener('click', () => {
  openItemModal(null, currentSection);
});
